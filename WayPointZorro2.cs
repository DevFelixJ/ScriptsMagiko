...
public class WayPointZorro2 : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other) // funcion ontrigger en momento que toca con algo
    {
        if (other.tag == "WayPoint") // si el objeto es = WayPoint
        {
            ...
            transform.LookAt(new Vector3(target.position.x, target.position.y, target.position.z)); // hacer lookAt para que mire al nuevo punto con vector 3 nuevas coordenadas del target, position y para que no vuele
        }
    }
}